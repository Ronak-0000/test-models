...
with col1:
    ...
    miss_dist = st.slider("Miss Distance (meters)", min_value=5.0, max_value=2500.0, value=75.0, step=5.0)
...
with col2:
    if run_btn:
        ...
        # Propagate exactly relative to TLE epoch to keep drift within realistic 24h envelope
        sat_a = Satrec.twoline2rv(l1_a, l2_a)
        sat_b = Satrec.twoline2rv(l1_b, l2_b)
        
        jd_epoch = sat_a.jdsatepoch
        fr_epoch = getattr(sat_a, 'jdsatepochF', getattr(sat_a, 'jdsatepochf', 0.0))
        
        # Propagate 2 hours post-epoch
        jd_target = jd_epoch
        fr_target = fr_epoch + (2.0 / 24.0)
        
        _, pos_a, vel_a = sat_a.sgp4(jd_target, fr_target)
        _, pos_b, vel_b = sat_b.sgp4(jd_target, fr_target)
        pos_a, vel_a = np.array(pos_a), np.array(vel_a)
        pos_b, vel_b = np.array(pos_b), np.array(vel_b)
        
        alt_a = float(np.linalg.norm(pos_a) - 6378.137)
        time_since_epoch = 2.0  # 2 hours
        inc_a = float(np.degrees(sat_a.inclo))
        ...
        with torch.no_grad():
            ...
            drift_raw = scaler_y.inverse_transform(pred_scaled)[0]
            
            # Physical sanity clamp: orbital drift is on the scale of meters/km
            dx_km = float(np.clip(drift_raw[0], -2.0, 2.0))
            dy_km = float(np.clip(drift_raw[1], -2.0, 2.0))
            dz_km = float(np.clip(drift_raw[2], -0.5, 0.5))
        
        refined_pos_a = pos_a + np.array([dx_km, dy_km, dz_km])
        
        # Run Model 2 Conjunction Assessment
        ...
        # 2D Conjunction Collision Probability Formulation
        ...
        collision_probability_pct = float(min(100.0, p_c_raw * 100.0))

        # Dashboard Top Metric Cards
        metric_col1, metric_col2, metric_col3 = st.columns(3)
        with metric_col1:
            badge_icon = "🟢" if pred_label == "LOW" else ("🟠" if pred_label == "MEDIUM" else "🔴")
            st.metric(label="Risk Classification", value=f"{badge_icon} {pred_label}")
        with metric_col2:
            st.metric(label="Collision Probability", value=f"{collision_probability_pct:.4f}%")
        with metric_col3:
            total_drift_m = float(np.linalg.norm([dx_km, dy_km, dz_km]) * 1000.0)
            st.metric(label="Total Predicted Drift", value=f"{total_drift_m:.2f} m")
            
        # 3D Plotly Visualization (Realistic Orbital Scale)
        fig = go.Figure()
        ...
        with res_col1:
            ...
            st.json({
                "Raw SGP4 ECI Position (km)": [round(float(p), 3) for p in pos_a],
                "Predicted Drift [dx, dy, dz] (meters)": [round(dx_km * 1000.0, 2), round(dy_km * 1000.0, 2), round(dz_km * 1000.0, 2)],
                "Refined ECI Position (km)": [round(float(p), 3) for p in refined_pos_a]
            })
        with res_col2:
            ...
            st.json({
                ...
                "Confidence Distribution": {
                    "LOW": f"{float(probs[0])*100:.2f}%",
                    "MEDIUM": f"{float(probs[1])*100:.2f}%",
                    "CRITICAL": f"{float(probs[2])*100:.2f}%"
                }
            })
    else:
